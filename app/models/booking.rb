class Booking < ApplicationRecord
 ...
 def generate_rand_str
  ...
  def val_quota
    ...
    num_user_bookings = Booking.where({user_id: user_id, facility_type_id: facility_type_id, booking_date: Date.today..(Date.today + 14.days)}).length
    if fac_quota<=num_user_bookings && facility_type_id != 4
      p "-------------------------"
      p 'this should fail'
      p "-------------------------"
      errors.add('booking count', "exceeded quota for #{FacilityType.find(facility_type_id).name} for the next 14 days")
      p errors
    else
      p "-------------------------"
      p 'this didnt fail'
